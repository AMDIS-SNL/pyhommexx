...
**Ownership:** this repo owns bindings, NN, data pipeline, training loop, BPTT harness, and CI. The C++ team owns adjoint kernels, `JtV` coverage, and FAD scalar types (§4).
...
4. **ne120 is where BPTT stops being a single-node problem.** A snapshot with tracers is ~22.6 GB; a two-entry tape does not fit on one GPU. Cost compounds twice over ne30 — 16× elements and 4× steps — so a fixed-wall-clock window shrinks ~64×. Distributed checkpoint placement stops being deferrable here.
...
| A3 | `Trilinos_ENABLE_ROL=ON` + PyROL in `configure-trilinos.sh`. Source build; no PyPI distribution | M | |
...
| ID | Task | Size |
|---|---|---|
| F1 | Spike: torch `state_dict` ↔ PyROL vector round-trip, before F2–F4 are scheduled | M |
| F2 | ROL objective wrapping coupled forward + backward | L |
| F3 | Serial training loop; Adam baseline for comparison | M |
| F4 | Distributed parameter-handling design note | M |
...
|---|---|---|
| M0 — Foundations | A2–A8, B10, B11, §4 + §9 questions sent | `ft` clarified; tracer registry designed; `ps`-drift question answered; ne30 config builds |
...
| M6 — Solver-in-the-loop | F1, F2 | Multi-step BPTT beats the M4 baseline |
...
| PyROL↔PyTorch integration overrun | Med-High | Med | F1 spike; Adam fallback keeps M4–M6 viable |
...
6. F1 — PyROL spike, early and parallel.
