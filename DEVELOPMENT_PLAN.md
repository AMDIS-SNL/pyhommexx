...
**Inference architecture: Python ping-pong first, C++ NN deferred.** Once M4 produces a satisfactory offline-trained NN, coupled runs drive it through the same `harness.py` ping-pong that trained it — the trained checkpoint becomes another `Backend` slot analogous to `StubBackend`. Same code path for training and inference means bit-for-bit reproducibility and instantaneous model iteration. Boundary-crossing cost is bounded at once-per-subcycle by the forcing-cadence decision; B8 measurements will quantify it. Moving to C++-owned inference (libtorch or ONNX Runtime called from `prim_advance_exp` in the slot `ForcingFunctor::states_forcing` occupies) is a separate 2–4 week port, gated on any of: (a) integration into the E3SM coupled driver, which is C++ and cannot invert control; (b) B8 measurements at ne120+ showing the ping-pong boundary dominating NN compute; (c) multi-node distributed inference where PyTorch's runtime conflicts with MPI-owned decomposition. None apply yet. **This framing depends on D5 holding.** If the NN acquires a horizontal receptive field, DSS becomes a real operator and its transpose enters the gradient (see DSS reconciliation, above), which changes both the training math and the boundary-crossing cost profile. Revisit ping-pong-vs-C++ if D5 is relaxed.
...
| D9 | C++ inference path for the trained NN: `NNPhysicsForcing` class called from `prim_advance_exp` in the slot `ForcingFunctor::states_forcing` occupies, loading a frozen TorchScript (or ONNX Runtime) checkpoint. Adds libtorch (or ORT) as a fork build dep; Kokkos↔ATen tensor interop on device pointers via `torch::from_blob` / equivalent. Gated per §2 "Inference architecture" — not for M4–M6. Triggers: E3SM coupled-driver integration, B8 measurements showing ping-pong dominates at ne120+, or multi-node distributed inference. Blocked if D5 is relaxed (horizontal-RF NN) — architecture would need revisit before this port | L |
